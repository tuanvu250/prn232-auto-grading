...
function getUserRoles(token: string | undefined): string[] {
  ...
  try {
    ...
    return normalizeRoles(Array.isArray(decoded.role) ? decoded.role : [decoded.role]);
  } catch {
    return [];
  }
}
...
export function proxy(request: NextRequest) {
  ...
  if (!token || userRoles.length === 0) {
    ...
    const response = NextResponse.redirect(new URL(ROUTE_MAP.login, request.url));
  ...
  if (hasRole(userRoles, ROLE_ADMIN)) {
    if (isAdminRoute || isPublicRoute(pathname)) return NextResponse.next();
